...
public class ManualTestBean {
    ...
    public void callController(){
        ...
        // Movie 5 Titel ändern
        String oldTitle = "Star Wars: Der Aufstieg Skywalkers";
        String newTitle = "Star Wars: Rise of the Skywalker";
        Movie movie5 = this.movieRepository.findAllMoviesByTitle(oldTitle).get(0);

        if (this.movieRepository.findAllMoviesByTitle(oldTitle).size() == 1) {
            log.info("Star Wars: Der Aufstieg Skywalkers gefunden");
            movie5.setTitle(newTitle);
            this.movieRepository.save(movie5);
        } else {
            log.warn("Film mit Titel " + newTitle + " nicht gefunden");
        }

        // Movie mit premierDate Jahr
        int premierYear = 2015;
        LocalDate premierDate = LocalDate.of(premierYear,12,31);

        this.movieRepository.findByPremiereDateGreaterThan(premierDate)
                .forEach(movie -> {
                    log.info("Movies after PremierYear " + premierDate.getYear());
                    log.info(movie.getTitle());
                    log.info(String.valueOf(movie.getPremiereDate()));
                });

        this.movieRepository.findAllMoviesByDurationAndImdbRating(2.5, 7.0)
                .forEach(movie -> {
                    log.info("Movies Duration: " + movie.getDuration()
                            + " Movie Title: " + movie.getTitle()
                            + " Movie Rating: " + movie.getImdbRating()
                    );
                });
